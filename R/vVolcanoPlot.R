#' VolcanoPlot for specific feature
...
#' @example
#' objec@misc$vDE <- vDE(object=object, features=features, min.cells=20, clusters=NULL)
#'
#' vVolcanoPlot(object=object, key="vDE", cluster=cluster, feature.by=feature.by, top_n=5)
#'
#' @author rstatistics
#' @export
vVolcanoPlot <- function(object = object, key = "vDE", cluster = cluster, feature.by = feature.by, top_n = NULL){
  if (is.null(top_n)){ top_n = 5 }
  x <- object@misc[[key]][[cluster]][[feature.by]]
  x$gene <- rownames(x)
  logFCcut <- 0.585
  pvalCut <- 1e-3
  adjPcut <- 1e-5
  ...
  p <- ggplot(data=x, aes(avg_logFC, -log10(p_val_adj), label = gene)) +
    ...
    labs(x=bquote(~Log[2]~"(fold change)"), y=bquote(~-Log[10]~italic("P-value")), title = paste0(feature.by, " in ", "Cluster-", cluster)) +
    ylim(c(ymin, ymax)) +
    scale_x_continuous(
